function [G1,C,impact,fmat,fwt,ywt,gev,eu,loose,Q1,Q2]=gensys(g0,g1,c,psi,pi,div)
...
% % end
if det(g0)==0
    fprintf('The existence is not certain.\n')
end
...
% ----------------------------------------
   ...
   if abs(a(i,i))<realsmall & abs(b(i,i))<realsmall
      zxz=1;  % if exist one i, s.t. both are really small, then zxz==1
   end
end
div ;
nunstab;
if ~zxz  
   [a b q z]=qzdiv(div,a,b,q,z);  %order
end
gev=[diag(a) diag(b)];  %
if zxz
...
if nunstab == 0
  ...
  md=min(size(deta));
  bigev=find(diag(deta(1:md,1:md))>realsmall);  %non-0 singular values' column indexes
  ueta=ueta(:,bigev);
  veta=veta(:,bigev);
